...
print("ONE", json.dumps(state.role_overlaps(proj, ["Database backup and restore tooling"])))  # shares only "database"
...
t_contains "a single shared common word does NOT flag (noise guard)" 'ONE []' "$OUT"
